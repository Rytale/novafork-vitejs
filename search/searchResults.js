$(document).ready(async function () {
  ...
  const displaySearchSuggestions = (results, query) => {
    ...
    if (results.length === 0) {
      $searchSuggestions
        .html('<div class="no-suggestions">No suggestions available</div>')
        .removeClass("hidden");
      return;
    }
    ...
    const suggestionsHTML = results
      .map((media) => {
        ...
        const mediaRating = media.vote_average
  ...
  const fetchSelectedMedia = async (mediaId, mediaType) => {
    try {
      ...
      if (media) {
        ...
        }?path=${encodeURIComponent(mediaType)}/${encodeURIComponent(mediaId)}`;
        window.history.pushState(
          { mediaId, mediaType, title: media.title || media.name },
          "",
          newUrl
        );
      } else {
    ...
    const resultsHTML = results
      .map((media) => {
        ...
        const ratingStars =
          ...
          "☆".repeat(5 - Math.round(media.vote_average / 2));

        return `
                <div class="media-card" data-id="${media.id}" data-type="${
          media.media_type
        }">
  ...
  const handleSearchInput = debounce(async () => {
    ...
    const result = await apiCall("search/multi", {
      query,
      include_adult: false,
    });
    if (result) {
