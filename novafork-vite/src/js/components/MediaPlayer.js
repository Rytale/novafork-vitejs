...
export class MediaPlayer {
    ...
    switch (provider) {
      ...
      case 'cinescrape':
        try {
          ...
          const response = await fetch(`https://scraper.cinescrape.com/tv/${mediaId}/${seasonId}/${episodeId}`);
          if (!response.ok) throw new Error('Network response was not ok');
          const data = await response.json();

          const tvSource = data.find(source => source.quality === '2160p' || source.quality === '1080p');
  ...
  showLoadingScreen(time = 2500) {
    ...
    const interval = setInterval(() => {
      if (currentProgress >= 100) {
        ...
        const messageIndex = Math.min(Math.floor(currentProgress / 20), this.loadingMessages.length - 1);
  ...
  async tryNextProvider(media, mediaType) {
    ...
    const providerName = this.providerNames[this.selectedProvider] || this.selectedProvider;
  ...
  async displayMedia(media, mediaType, isRetry = false) {
    ...
    try {
      ...
      if (mediaType === 'movie') {
        ...
        embedUrl = await this.getTvEmbedUrl(media.id, seasonId, episodeId, provider);
      }

      if (!isRetry && DEV_CONFIG.showLoadingAnimation && DEV_CONFIG.loadingProviders.includes(provider)) {
      ...
      if (!videoLoaded) {
        console.log(`Provider ${provider} failed to load video, trying next provider...`);
  ...
  attemptFullscreenAndLockOrientation(element) {
    ...
    if (!orientationLockEnabled) return;
